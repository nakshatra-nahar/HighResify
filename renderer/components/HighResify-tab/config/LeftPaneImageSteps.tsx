...
interface IProps {
  ...
  HighResifyHandler: () => Promise<void>;
  batchMode: boolean;
  setBatchMode: React.Dispatch<React.SetStateAction<boolean>>;
  imagePath: string;
  doubleHighResify: boolean;
  setDoubleHighResify: React.Dispatch<React.SetStateAction<boolean>>;
  dimensions: {
    width: number | null;
    height: number | null;
  };
...
function LeftPaneImageSteps({
  ...
  }, [dimensions.width, dimensions.height, doubleHighResify, scale]);
    ...
    >
      ...
      <div className="flex flex-row items-center gap-2">
        ...
        <p
          ...
          data-tooltip-content="This will let you HighResify all files in a folder at once"
        >
          Batch HighResify
        </p>
      </div>
        ...
        {!batchMode && (
          <div className="flex items-center gap-1">
            <input
              type="checkbox"
              className="checkbox"
              checked={doubleHighResify}
              onChange={(e) => {
                if (e.target.checked) {
                  setDoubleHighResify(true);
                } else {
            ...
            <p
              ...
              onClick={(e) => {
                setDoubleHighResify(!doubleHighResify);
              }}
            >
              Double HighResify
            </p>
            <button
              ...
              data-tooltip-id="tooltip"
              data-tooltip-content="Enable this option to run HighResify twice on an image. Note that this may cause a significant increase in processing time and possibly performance issues for scales greater than 4X."
            >
              ?
            </button>
          </div>
          ...
          <div className="step-heading flex items-center gap-2">
            ...
            {featureFlags.APP_STORE_BUILD && (
              <button
                ...
                onClick={() =>
                  alert(
                    "Due to MacOS App Store security restrictions, HighResify requires you to select an output folder everytime you start it.\n\nTo avoid this, you can permanently save a default output folder in the HighResify 'Settings' tab.",
                  )
                }
              >
                ?
      ...
      <div className="animate-step-in">
        ...
        >
          {progress.length > 0 ? "HighResifying⏳" : "HighResify"}
        </button>
      </div>
