...
import ProgressBar from "../components/HighResify-tab/view/ProgressBar";
import RightPaneInfo from "../components/HighResify-tab/view/RightPaneInfo";
import ImageOptions from "../components/HighResify-tab/view/ImageOptions";
import LeftPaneImageSteps from "../components/HighResify-tab/config/LeftPaneImageSteps";
...
import { HighResifyCloudModal } from "../components/HighResifyCloudModal";
import { featureFlags } from "@common/feature-flags";
import {
  BatchHighResifyPayload,
  DoubleHighResifyPayload,
  ImageHighResifyPayload,
...
const Home = () => {
  ...
  const [doubleHighResify, setDoubleHighResify] = useState(false);
  const overwrite = useAtomValue(overwriteAtom);
  const [upscaledBatchFolderPath, setUpscaledBatchFolderPath] = useState("");
  const [doubleHighResifyCounter, setDoubleHighResifyCounter] = useState(0);
  ...
  useEffect(() => {
    // HighResify VERSION
    const HighResifyVersion = navigator?.userAgent?.match(
      /HighResify\/([\d\.]+\d+)/,
    )[1];
    setVersion(HighResifyVersion);
  }, []);
  ...
  useEffect(() => {
    const handleErrors = (data: string) => {
      if (data.includes("Invalid GPU")) {
        toast({
          ...
          action: (
            <div className="flex flex-col gap-2">
              ...
              <a href="https://docs.HighResify.org/" target="_blank">
        ...
        toast({
          ...
          description: `HighResify encountered an error. Possibly, the HighResify binary failed to execute the commands properly. Try checking the logs to see if you get any information. You can post an issue on HighResify's GitHub repository for more help.`,
        });
        resetImagePaths();
      }
    };
    ...
    // HighResify ERROR
    window.electron.on(COMMAND.HighResify_ERROR, (_, data: string) => {
      toast({
        title: "Error",
        description: data,
      });
    ...
    // HighResify PROGRESS
    window.electron.on(COMMAND.HighResify_PROGRESS, (_, data: string) => {
      if (data.length > 0 && data.length < 10) {
        setProgress(data);
      } else if (data.includes("converting")) {
        setProgress("Scaling and converting image...");
      } else if (data.includes("Successful")) {
        setProgress("HighResify Successful!");
      }
      handleErrors(data);
      logit(`🚧 HighResify_PROGRESS: `, data);
    });
    // FOLDER HighResify PROGRESS
    window.electron.on(COMMAND.FOLDER_HighResify_PROGRESS, (_, data: string) => {
      ...
      handleErrors(data);
      logit(`🚧 FOLDER_HighResify_PROGRESS: `, data);
    });
    // DOUBLE HighResify PROGRESS
    window.electron.on(COMMAND.DOUBLE_HighResify_PROGRESS, (_, data: string) => {
      if (data.length > 0 && data.length < 10) {
        if (data === "0.00%") {
          setDoubleHighResifyCounter(doubleHighResifyCounter + 1);
        }
        setProgress(data);
      }
      handleErrors(data);
      logit(`🚧 DOUBLE_HighResify_PROGRESS: `, data);
    });
    // HighResify DONE
    window.electron.on(COMMAND.HighResify_DONE, (_, data: string) => {
      setProgress("");
      setUpscaledImagePath(data);
      logit("upscaledImagePath: ", data);
      logit(`💯 HighResify_DONE: `, data);
    });
    // FOLDER HighResify DONE
    window.electron.on(COMMAND.FOLDER_HighResify_DONE, (_, data: string) => {
      setProgress("");
      setUpscaledBatchFolderPath(data);
      logit(`💯 FOLDER_HighResify_DONE: `, data);
    });
    // DOUBLE HighResify DONE
    window.electron.on(COMMAND.DOUBLE_HighResify_DONE, (_, data: string) => {
      setProgress("");
      setTimeout(() => setUpscaledImagePath(data), 500);
      setDoubleHighResifyCounter(0);
      logit(`💯 DOUBLE_HighResify_DONE: `, data);
    });
  ...
  useEffect(() => {
    ...
    try {
      fetch("https://raw.githubusercontent.com/HighResify/HighResify/main/news.md", {
        cache: "no-cache",
      })
        .then((res) => {
          return res.text();
      ...
      console.log("Could not fetch HighResify News");
    }
  }, [news]);
  ...
  const HighResifyHandler = async () => {
    ...
    if (imagePath !== "" || batchFolderPath !== "") {
      ...
      // Double HighResify
      if (doubleHighResify) {
        window.electron.send<DoubleHighResifyPayload>(COMMAND.DOUBLE_HighResify, {
          imagePath,
          outputPath,
          model,
        ...
        logit("🏁 DOUBLE_HighResify");
      } else if (batchMode) {
        // Batch HighResify
        setDoubleHighResify(false);
        window.electron.send<BatchHighResifyPayload>(COMMAND.FOLDER_HighResify, {
          batchFolderPath,
          outputPath,
          model,
          gpuId: gpuId.length === 0 ? null : gpuId,
        ...
        logit("🏁 FOLDER_HighResify");
      } else {
        // Single Image HighResify
        window.electron.send<ImageHighResifyPayload>(COMMAND.HighResify, {
          imagePath,
          outputPath,
          model,
          gpuId: gpuId.length === 0 ? null : gpuId,
  ...
  return (
    <div className="flex h-screen w-screen flex-row overflow-hidden bg-base-300">
      ...
      >
        ...
        {/* HighResify CLOUD MODAL */}
        {featureFlags.SHOW_HighResify_CLOUD_INFO && (
          <HighResifyCloudModal
            show={showCloudModal}
        ...
        {!dontShowCloudModal && featureFlags.SHOW_HighResify_CLOUD_INFO && (
          <button
        ...
        {selectedTab === 0 && (
          <LeftPaneImageSteps
            ...
            HighResifyHandler={HighResifyHandler}
            batchMode={batchMode}
            setBatchMode={setBatchMode}
            imagePath={imagePath}
            doubleHighResify={doubleHighResify}
            setDoubleHighResify={setDoubleHighResify}
      ...
      >
        ...
        upscaledBatchFolderPath.length === 0 ? (
          <ProgressBar
            batchMode={batchMode}
            progress={progress}
            doubleHighResifyCounter={doubleHighResifyCounter}
