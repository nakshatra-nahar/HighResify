...
import {
  savedBatchHighResifyFolderPath,
  setSavedBatchHighResifyFolderPath,
...
const selectFolder = async (event, message) => {
  ...
  const {
    ...
    defaultPath: savedBatchHighResifyFolderPath,
  ...
  if (canceled) {
    ...
    setSavedBatchHighResifyFolderPath(folderPaths[0]);
    logit("📁 Selected Folder Path: ", savedBatchHighResifyFolderPath);
