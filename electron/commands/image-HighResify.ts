...
import { ImageHighResifyPayload } from "../../common/types/types";
...
const imageHighResify = async (event, payload: ImageHighResifyPayload) => {
  ...
  if (fs.existsSync(outFile) && !overwrite) {
    // If already HighResifyed, just output that file
    logit("✅ Already HighResifyed at: ", outFile);
    mainWindow.webContents.send(COMMAND.HighResify_DONE, outFile);
  } else {
    logit(
      "✅ HighResify Variables: ",
      JSON.stringify({
        model,
        gpuId,
        saveImageAs,
    ...
    const onData = (data: string) => {
      ...
      data = data.toString();
      mainWindow.webContents.send(COMMAND.HighResify_PROGRESS, data.toString());
    ...
    const onError = (data) => {
      ...
      mainWindow.webContents.send(COMMAND.HighResify_ERROR, data.toString());
      failed = true;
      HighResify.kill();
      return;
    };
    const onClose = async () => {
      if (!failed && !stopped) {
        ...
        mainWindow.setProgressBar(-1);
        mainWindow.webContents.send(COMMAND.HighResify_DONE, outFile);
        showNotification("HighResify", "Image HighResifyed successfully!");
      }
    };
...
export default imageHighResify;
