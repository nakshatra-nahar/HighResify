...
/**
 * The saved folder path so that the select folder to HighResify dialog can open to the last used path.
 */
export let savedBatchHighResifyFolderPath: string | undefined = undefined;
export function setSavedBatchHighResifyFolderPath(
  value: string | undefined,
): void {
  savedBatchHighResifyFolderPath = value;
  logit("📁 Updating Folder Path: ", savedBatchHighResifyFolderPath);
}
...
/**
 * The stopped variable to stop the batch HighResify process.
 */
export let stopped = false;

/**
 * The child processes array to store the spawned HighResify processes.
 */
...
export function fetchLocalStorage(): void {
  ...
  mainWindow.webContents
    .executeJavaScript(
      'localStorage.getItem("lastSavedBatchHighResifyFolderPath");',
      true,
    )
    .then((lastSavedBatchHighResifyFolderPath: string | null) => {
      if (
        lastSavedBatchHighResifyFolderPath &&
        lastSavedBatchHighResifyFolderPath.length > 0
      ) {
        setSavedBatchHighResifyFolderPath(lastSavedBatchHighResifyFolderPath);
      }
    });
