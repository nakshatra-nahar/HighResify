...
assets_url=$(curl -s https://api.github.com/repos/HighResify/HighResify-ncnn/releases/latest | jq -r '.assets_url')
...
for folder in extracted_files/HighResify-bin-*; do
	if [[ -d $folder ]]; then
		platform=$(echo "extracted_files/$folder" | cut -d '-' -f 5)
		echo "Moving files in $folder to $platform folder..."
		if [[ "$platform" == "linux" ]]; then
			cp "$folder"/HighResify-bin resources/linux/bin/HighResify-bin
		elif [[ "$platform" == "macos" ]]; then
			cp "$folder"/HighResify-bin resources/mac/bin/HighResify-bin
		elif [[ "$platform" == "windows" ]]; then
			cp "$folder"/HighResify-bin.exe resources/win/bin/HighResify-bin.exe
